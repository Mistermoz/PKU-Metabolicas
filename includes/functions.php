...
/****************** Calcula la edad (formato: dia-mes-a�o) ************************/
function sacar_edad($fecha){
	if($fecha != "") {
	    ...
	    $anos = (string)($y2 - ($y1 + 1)); // calculamos a�os
...
function edita_pacientes(){
	...
	$query = "SELECT * FROM paciente WHERE Nombre='".$nom_paciente."' AND Fecha_lectura='".$flectura."'";
	$content = $dbh->get_results( $query );
	if ( count($content) > 0 ) {
		foreach ( $content as $row ) {
			$aDatos = array();
...
function update_pacientes(){
	...
	$mi_tabla = 'paciente';
	$query = "SELECT * FROM $mi_tabla WHERE F_lectura='".$flectura."'";
	$content = $dbh->get_results( $query );
	if ( count($content) > 0 ) {
		foreach ( $content as $row ) {
			$aDatos = array();
...
add_action('wp_ajax_update_pacientes', 'update_pacientes');
add_action('wp_ajax_nopriv_update_pacientes', 'update_pacientes');
/*************** Funcion Consulta Fechas por paciente *****************************/
function fechas(){
	global $dbh;
	$nom_paciente = $_POST['nom_paciente'];
	$mi_tabla = 'paciente';
	$query = "SELECT * FROM $mi_tabla WHERE Nombre='".$nom_paciente."' ORDER BY F_lectura DESC";
	$content = $dbh->get_results( $query );
	if ( count($content) > 0 ) {
		$aDatos = array();
		foreach ( $content as $row ) {
			$aDatos[] = array( 'fechas' => ''.$row->F_lectura.'', 'fechas_dos' => ''.$row->Fecha_lectura.'');
		}
	}else{
		$aDatos = array();
...
add_action('wp_ajax_fechas', 'fechas');
add_action('wp_ajax_nopriv_fechas', 'fechas');
...
add_action('wp_ajax_ingreso_pacientes', 'ingreso_pacientes');
add_action('wp_ajax_nopriv_ingreso_pacientes', 'ingreso_pacientes');
...
function busca_pacientes(){
	...
	if ( count($content) > 0 ) {
	   $aDatos = array();
		...
		$aDatos = array();
		$aDatos[] = array(  'nombres' => 'Sin Datos');
	}
    echo json_encode($aDatos);
	die();
}
add_action('wp_ajax_busca_pacientes', 'busca_pacientes');
add_action('wp_ajax_nopriv_busca_pacientes', 'busca_pacientes');

/***************************** Funcion Actuliza pacientes (Pacientes)  *******************************/
function actualiza_pacientes(){
	global $dbh;
	$nom_paciente = $_POST['nom_paciente'];
	$flectura = $_POST['flectura'];
	$fenil = $_POST['fenil'];
  ...
  $content = $dbh->query( $query );
	if ( $content > 0 ) {
			$aDatos = array();
			$aDatos[] = array( 'Paciente' => 'Paciente Actualizado');
	}else{
		$aDatos = array();
		$aDatos[] = array(  'Paciente' => 'Erro');
	}
    echo json_encode($aDatos);
	die();
}
add_action('wp_ajax_actualiza_pacientes', 'actualiza_pacientes');
add_action('wp_ajax_nopriv_actualiza_pacientes', 'actualiza_pacientes');

/***************************** Funcion Actulizar pacientes (Indicaciones)  *******************************/
function actualizar_pacientes(){
  ...
  $id = $_POST['id'];
  $estado = $_POST['estado'];
  $pmuestra = $_POST['pmuestra'];
  $fleche = $_POST['fleche'];
  $fcontrol = $_POST['fcontrol'];
  $count = sizeof($nom_paciente);
  $aDatos = array();
  for($x = 0; $x < $count; $x++) {
    $query = 'UPDATE paciente SET Estado="'.$estado[$x].'", prox_lectura="'.$pmuestra[$x].'", fecha_entrega_leche="'.$fleche[$x].'", fecha_control="'.$fcontrol[$x].'" WHERE id="'.$id[$x].'"';
    $content = $dbh->query( $query );
    if ( $content > 0 ) {
        $j = $x + 1;
        $aDatos[$x] = 'Paciente '.$j.' actualizado';
    }else{
      $j = $x + 1;
      $aDatos[$x] = 'Paciente '.$j.' error';
    }
  }
    echo json_encode($aDatos);
  die();
...
add_action('wp_ajax_actualizar_pacientes', 'actualizar_pacientes');
add_action('wp_ajax_nopriv_actualizar_pacientes', 'actualizar_pacientes');

/***************************** Funcion Actulizar Registros (Editar registros)  *******************************/
...
add_action('wp_ajax_actualizar_registros', 'actualizar_registros');
add_action('wp_ajax_nopriv_actualizar_registros', 'actualizar_registros');

/********************************* Busca datos paciente para editar ****************************/
...
function actualiza_paciente(){
  ...
  if ( $content > 0 ) {
    ...
    $aDatos = array();
    $aDatos[] = array(  'Paciente' => 'Error');
  }
    echo json_encode($aDatos);
  die();
}
add_action('wp_ajax_actualiza_paciente', 'actualiza_paciente');
add_action('wp_ajax_nopriv_actualiza_paciente', 'actualiza_paciente');

/***************************** Funcion Borrar pacientes (Editar)  *******************************/
function borrar_paciente(){
	global $dbh;
  $nombre_a = $_POST['nombre_antiguo'];
  $query = 'DELETE from paciente WHERE Nombre="'.$nombre_a.'"';
  $content = $dbh->query( $query );
  if ( $content > 0 ) {
      $aDatos = array();
      $aDatos[] = array( 'Paciente' => 'Paciente Borrado');
  }else{
    $aDatos = array();
    $aDatos[] = array(  'Paciente' => 'Error');
  }
...
add_action('wp_ajax_borrar_paciente', 'borrar_paciente');
add_action('wp_ajax_nopriv_borrar_paciente', 'borrar_paciente');

/***************************** Funcion Eliminar Registros (Editar registros)  *******************************/
function eliminar_registros(){
  ...
  $aDatos = array();
  for($x = 0; $x < $count; $x++) {
    $query = 'DELETE from paciente WHERE id="'.$id[$x].'"';
    $content = $dbh->query( $query );
    if ( $content > 0 ) {
        $aDatos[$x] = 'Registro '.$id[$x].' eliminado';
    }else{
      $j = $x + 1;
      $aDatos[$x] = 'Registro '.$id[$j].' error';
    }
  }
    echo json_encode($aDatos);
  die();
...
add_action('wp_ajax_eliminar_registros', 'eliminar_registros');
add_action('wp_ajax_nopriv_eliminar_registros', 'eliminar_registros');
?>